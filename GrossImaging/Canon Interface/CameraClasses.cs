...
namespace GrossImaging
{
    public class SDKHandler : IDisposable
    {
            ...
            set
            {
                switch (value)
                {
                    case EDSDK.EDS_ERR_OK:
                        break;
                    default:
                        System.Diagnostics.Debug.WriteLine("Error:" + value.ToString());
        ...
        public delegate void ImageAcquiredHandler(Image image);
        public delegate void CaptureErrorHandler(EosErrorCode error);
        ...
        public event EventHandler CameraWillShutdownSoon;
        public event ImageAcquiredHandler ImageAcquired;
        public event CaptureErrorHandler CaptureError;
        ...
        private uint Camera_SDKObjectEvent(uint inEvent, IntPtr inRef, IntPtr inContext)
        {
            ...
            switch (inEvent)
            {
                ...
                case EDSDK.ObjectEvent_DirItemRequestTransfer:
                    ...
                    {
                        ImageAcquired(DownloadImage2(inRef));
                    }
        ...
        public Bitmap DownloadImage2(IntPtr ObjectPointer)
        {
            Bitmap result;
            lock (LVlock)
            {
                ...
                error = EDSDK.EdsGetDirectoryItemInfo(ObjectPointer, out directoryItemInfo);
                ...
                if (error == EDSDK.EDS_ERR_OK)
                {
                    error = EDSDK.EdsDownload(ObjectPointer, directoryItemInfo.Size, stream);
                }
                ...
                result = (Bitmap)System.Drawing.Image.FromStream(memoryStream);
        ...
        private uint Camera_SDKStateEvent(uint inEvent, uint inParameter, IntPtr inContext)
        {
            ...
            switch (inEvent)
            {
                ...
                case EDSDK.StateEvent_CaptureError:
                    ...
                    {
                        EosErrorCode error = (EosErrorCode)inParameter;
                        CaptureError(error);
                    }
                    break;
                ...
                case EDSDK.StateEvent_WillSoonShutDown:
                    if (CameraWillShutdownSoon != null) CameraWillShutdownSoon(this, new EventArgs());
                    break;
            }
            return EDSDK.EDS_ERR_OK;
        }
        ...
        public void SetSetting(uint PropID, uint Value)
        {
            if (MainCamera.Ref != IntPtr.Zero)
            {
                lock (LVlock)
                {
                    ...
                    System.Diagnostics.Debug.WriteLine("Attempt to set property: " + PropID.ToString() + " to value " + Value.ToString());
        ...
        bool _pauseLiveViewRequested = false;
        ...
        private void DownloadEvf()
        {
            LVThread = new Thread(delegate()
            {
                ...
                while (IsLiveViewOn)
                {
                //    System.Diagnostics.Debug.WriteLine("DownloadEVF acquiring lock");

                    lock (LVlock)
                    {
                  //      System.Diagnostics.Debug.WriteLine("DownloadEVF has the lock");
               ...
               //     System.Diagnostics.Debug.WriteLine("DownloadEVF released lock");
            ...
            new Thread(delegate()
         {
             ...
             lock (LVlock)
             {
                 ...
                 while (BusyCount < 20)
                 {
                     ...
                     err = EDSDK.EdsSendCommand(MainCamera.Ref, EDSDK.CameraCommand_PressShutterButton, (uint)state);
        ...
        public void PressShutterButtonOrg(EDSDK.EdsShutterButton state)
        {
            lock (LVlock)
            {
                Error = EDSDK.EdsSendCommand(MainCamera.Ref, EDSDK.CameraCommand_PressShutterButton, (uint)state);
            }
        }
        ...
        {
            lock (LVlock) Error = EDSDK.EdsSendCommand(MainCamera.Ref, EDSDK.CameraCommand_ExtendShutDownTimer,0);
        }
        ...
        public void TakePhoto()
        {
            ...
            new Thread(delegate()
            {
                int BusyCount = 0;
                uint err = EDSDK.EDS_ERR_OK;
              //  System.Diagnostics.Debug.WriteLine("TakePhoto acquiring lock");

                lock (LVlock)
                {
                 //   System.Diagnostics.Debug.WriteLine("TakePhoto has the lock");
                    ...
                    while (BusyCount < 50)
                    {
                        ...
                        if (err == EDSDK.EDS_ERR_DEVICE_BUSY)
                        {
                            System.Diagnostics.Debug.WriteLine("ERR_BUSY");
          ...
          //      System.Diagnostics.Debug.WriteLine("TakePhoto released lock");
